...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const attendee = await findAttendeeByPhone(phoneClean);
    if (!attendee) {
    ...
    await pool.query(
      `INSERT INTO myapp_otp_tokens (phone, otp_hash, expires_at) VALUES (?, ?, ?)`,
    ...
    return res.status(500).json({ error: 'Error interno al enviar el código' });
  }
}
