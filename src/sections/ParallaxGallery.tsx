          ...
          <div ref={galleryTrackRef} className="flex items-center gap-8 h-full px-12 pt-24 will-change-transform">
            {images.map((image, index) => (
              <div key={image.id}
                ...
                onClick={() => hasVideo ? openVideo(image, index) : openPhoto(index)}>
                ...
                <div className="relative w-[450px] h-[300px] overflow-hidden rounded-xl">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                        image.transition === 'morphing'
                          ? 'border-aira-lime/40 bg-aira-lime/10 group-hover:border-aira-lime group-hover:bg-aira-lime/20 group-hover:scale-110'
                          : 'border-white/30 bg-white/5 group-hover:border-white/70 group-hover:bg-white/15 group-hover:scale-125'
                      }`}>
                        <Play className={`w-6 h-6 ml-1 transition-colors duration-300 ${
                          image.transition === 'morphing' ? 'text-aira-lime/70 group-hover:text-aira-lime' : 'text-white/60 group-hover:text-white'
                        }`}/>
                      </div>
                    </div>
                  )}
                  ...
                  {hasVideo && (
                    <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <span className={`font-mono-custom text-[8px] uppercase tracking-widest px-2 py-1 rounded-full border ${
                        image.transition === 'morphing'
                          ? 'border-aira-lime/30 bg-aira-lime/10 text-aira-lime/70'
                          : 'border-white/20 bg-white/5 text-white/50'
                      }`}>
                        {image.transition === 'morphing' ? '⬡ Portal' : '⊕ Zoom'}
                      </span>
                    </div>
                  )}
