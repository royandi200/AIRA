...
const Footer = () => {
  if (!footerConfig.brandName && !footerConfig.heroTitle && footerConfig.socialLinks.length === 0) return null;
    ...
    const ctx = gsap.context(() => {
      if (titleRef.current && portraitRef.current) {
        const st = ScrollTrigger.create({
          ...
          onUpdate: (self) => { if (titleRef.current) gsap.set(titleRef.current, { y: -self.progress * 100 }); },
        });
        scrollTriggerRefs.current.push(st);
      }
    }, sectionRef);
    return () => { ctx.revert(); scrollTriggerRefs.current.forEach(st => st.kill()); scrollTriggerRefs.current = []; };
  }, []);
    ...
    <section id="contact" ref={sectionRef} className="relative w-full bg-aira-darkBlue overflow-hidden">
      <div className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div ref={titleRef} className="relative z-10 text-center will-change-transform">
          <img src="/AIRA BLANCO.png" alt="AIRA" className="w-[55vw] max-w-2xl mx-auto object-contain drop-shadow-2xl" style={{ filter: 'drop-shadow(0 0 60px rgba(225,254,82,0.15))' }} />
          <p className="font-mono-custom text-lg text-aira-lime/60 uppercase tracking-[0.5em] mt-4">{footerConfig.heroSubtitle}</p>
        </div>
      ...
      <div className="relative bg-aira-darkBlue py-20 px-6 md:px-12">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
            <div>
              <div className="flex items-center gap-3 mb-6"><img src="/AIRA BLANCO.png" alt="AIRA Logo" className="h-12 w-auto" /></div>
            ...
            <div>
              ...
              <ul className="space-y-3">
                {footerConfig.quickLinks.map((link) => (
                  <li key={link}><a href="#" className="text-sm text-white/50 hover:text-aira-lime transition-colors flex items-center gap-2 group"><span>{link}</span><ExternalLink className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" /></a></li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-4">
                <li className="flex items-start gap-3"><Mail className="w-4 h-4 text-aira-lime/60 mt-0.5" /><div><p className="text-sm text-white/50">{footerConfig.emailLabel}</p><a href={`mailto:${footerConfig.email}`} className="text-sm text-white hover:text-aira-lime transition-colors">{footerConfig.email}</a></div></li>
                <li className="flex items-start gap-3"><Phone className="w-4 h-4 text-aira-lime/60 mt-0.5" /><div><p className="text-sm text-white/50">{footerConfig.phoneLabel}</p><span className="text-sm text-white">{footerConfig.phone}</span></div></li>
                <li className="flex items-start gap-3"><MapPin className="w-4 h-4 text-aira-lime/60 mt-0.5" /><div><p className="text-sm text-white/50">{footerConfig.addressLabel}</p><span className="text-sm text-white">{footerConfig.address}</span></div></li>
              </ul>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button className="px-4 py-3 bg-aira-lime text-aira-darkBlue rounded-lg text-sm font-medium hover:bg-aira-lime/80 transition-colors">{footerConfig.newsletterButtonText}</button>
              </div>
            </div>
          </div>
          ...
          <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-6">{footerConfig.bottomLinks.map((link) => (<a key={link} href="#" className="text-xs text-white/30 hover:text-white/60 transition-colors">{link}</a>))}</div>
          </div>
        </div>
      </div>
    </section>
