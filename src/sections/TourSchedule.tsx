...
const TourSchedule = () => {
  ...
  return (
    <section id="booking" ref={sectionRef} className="relative w-full min-h-screen bg-aira-blue py-20 overflow-hidden">
      ...
      <div ref={contentRef} className="relative z-20 max-w-7xl mx-auto px-6 md:px-12">
        <div className="mb-16">
          <p className="font-mono-custom text-xs text-white/60 uppercase tracking-wider mb-2">{tourScheduleConfig.sectionLabel}</p>
          <h2 className="font-display text-5xl md:text-7xl text-white">{tourScheduleConfig.sectionTitle}</h2>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {TOUR_DATES.length > 0 && (
            <div className="hidden lg:flex lg:items-center">
              <div className="sticky top-32 w-full aspect-[4/3] rounded-2xl overflow-hidden bg-aira-darkBlue/20 border-2 border-aira-lime/30">
                <img src={TOUR_DATES[activeVenue]?.image} alt={TOUR_DATES[activeVenue]?.venue} className="w-full h-full object-cover transition-opacity duration-500" />
          ...
          <div className="space-y-4">
            {TOUR_DATES.map((tour, index) => {
              ...
              return (
                <div key={tour.id} className="tour-item group relative p-6 rounded-xl bg-white/80 backdrop-blur-sm border border-aira-lime/20 hover:bg-white hover:border-aira-lime/50 transition-all duration-300 cursor-pointer"
                  onMouseEnter={() => setActiveVenue(index)} onMouseLeave={() => setActiveVenue(0)}>
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                    <div className="flex-shrink-0 w-28">
                      <p className="font-mono-custom text-2xl font-bold text-aira-darkBlue">{tour.date.split('.').slice(1).join('.')}</p>
                      <p className="font-mono-custom text-xs text-aira-darkBlue/50">{tour.date.split('.')[0]}</p>
                    </div>
                    ...
                    <div className="flex-shrink-0">
                      {tour.status === 'on-sale' ? (
                        <button className="flex items-center gap-2 px-4 py-2 bg-aira-lime text-aira-darkBlue rounded-full text-sm font-medium hover:bg-aira-lime/80 transition-colors">
                          <Ticket className="w-4 h-4" /><span>{tourScheduleConfig.buyButtonText}</span>
                        </button>
                      ) : (
                        <button className="flex items-center gap-2 px-4 py-2 border border-aira-darkBlue/20 text-aira-darkBlue/60 rounded-full text-sm hover:border-aira-darkBlue/40 transition-colors">
                          <ExternalLink className="w-4 h-4" /><span>{tourScheduleConfig.detailsButtonText}</span>
                        </button>
                      )}
                    </div>
                  </div>
        ...
        <div className="mt-20 text-center">
          ...
          <button className="px-8 py-4 bg-aira-lime text-aira-darkBlue font-display text-sm uppercase tracking-wider rounded-full hover:bg-white transition-colors">{tourScheduleConfig.bottomCtaText}</button>
        </div>
      </div>
